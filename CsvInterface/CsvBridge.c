...
/**
 * Convert Fortran intensity estimate to OCaml CSV structure and call ocamlOut.
 *
 ...
 * @pre initOCaml() must be called first
 * @pre OCaml "ocamlOut" function registered via Callback.register
 * @pre est->qVals and est->iVals must have est->size elements each
 * @pre pth must be null-terminated
 * @pre No headers in arrays - function adds them
 */
void fortran2ocaml(estimate *est, char *pth) {
    ...
    for (int i = est->size - 1; i >= 0; i--) {
        ...
        snprintf(buffer, MAX_DOUBLE_STR, "%f", est->iVals[i]);
        ...
        // add q value to row
        snprintf(buffer, MAX_DOUBLE_STR, "%f", est->qVals[i]);
        cns_opi = caml_alloc(2, 0);
        Store_field(cns_opi, 0, caml_copy_string(buffer));
        Store_field(cns_opi, 1, row_lst);
        row_lst = cns_opi; 
    ...
    if (closure == NULL) {
        closure = caml_named_value("ocamlOut");
    }
    if (closure != NULL) {
