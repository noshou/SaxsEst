...
!  Modified:
!
!    09 January  2006 (Original; John Burkardt)
!    21 November 2025 (Small update Nathan Ouedraogo)
!    03 January  2026 (Fixed output formatting; Nathan Ouedraogo) 
!
program pdb_to_xyz
    ...
    character ( len = 255 ) mol_name
    integer ( kind = 4 ) dot_pos
...
!
    ...
    if ( 1 <= numarg ) then
        ...
        iarg = 1
        call get_command_argument ( iarg, pdb_file_name )
    ...
    if ( 2 <= numarg ) then
        ...
        iarg = 2
        call get_command_argument ( iarg, xyz_file_name )
...
!
!  Extract molecule name from PDB filename.
!
    dot_pos = index(pdb_file_name, '.', back=.true.)
    if (dot_pos > 1) then
        mol_name = pdb_file_name(1:dot_pos-1)
    else
        mol_name = pdb_file_name
    end if
!
!  Open the PDB file.
!
...
!
!    Input, integer ( kind = 4 ) XYZ_FILE_UNIT, the FORTRAN unit number
!    associated with the XYZ file.
!
!    Output, integer ( kind = 4 ) PDB_FILE_LINE_NUM, the number of lines read
!    from the PDB file.
!
...
!
    ...
    max_atoms = 100000
    allocate(x(max_atoms), y(max_atoms), z(max_atoms))
    allocate(element(max_atoms))
    ...
    do
        ...
        if ( s_eqi ( string(1:6), 'ENDMDL' )  ) then
            ...
            read ( string(31:54), '(3f8.3)', iostat = ios ) tmp_x, tmp_y, tmp_z
            ...
            y(atom_num) = tmp_y
            z(atom_num) = tmp_z
            element(atom_num) = tmp_element

        end if

    end do

    write ( xyz_file_unit, '(a)' ) trim(mol_name)
    write ( xyz_file_unit, '(i0)' ) atom_num

    do i = 1, atom_num
        write ( xyz_file_unit, '(a2,a1,f14.6,a1,f14.6,a1,f14.6)' ) &
            element(i), char(9), x(i), char(9), y(i), char(9), z(i)
    end do

    deallocate(x, y, z, element)
